    ...
    if (mode == MODE_INTR || mode == MODE_STEP || mode == MODE_SLNT || mode == MODE_DEMO)
    {
       ...
       const int MAX_TIMESTEPS = 5000;  // Safety limit
        ...
        while (true)
        {
            ...
            // Update GUI to show newly arrived orders in WAIT region
            FillDrawingList();
...
void Restaurant::LoadInputFile(const string& filename)
{
    ...
    // Create Normal cooks (IDs: 1 to N)
    for (int i = 1; i <= N; i++)
    {
    ...
    // Create Vegan cooks (IDs: N+1 to N+G)
    for (int i = 1; i <= G; i++)
    {
        Cook* newCook = new Cook(N + i, COOK_VGAN, SG, BO, BG);
        veganCooks.InsertEnd(newCook);
    }
    
    // Create VIP cooks (IDs: N+G+1 to N+G+V)
    for (int i = 1; i <= V; i++)
    {
        Cook* newCook = new Cook(N + G + i, COOK_VIP, SV, BO, BV);
...
void Restaurant::FillDrawingList()
{
    ...
    // Add all cooks to drawing list (COOK_REG region)
    Node<Cook*>* pCook;
    pCook = normalCooks.getHead(); while (pCook) { pGUI->AddToDrawingList(pCook->getItem()); pCook = pCook->getNext(); }
    pCook = veganCooks.getHead();  while (pCook) { pGUI->AddToDrawingList(pCook->getItem()); pCook = pCook->getNext(); }
    pCook = vipCooks.getHead();    while (pCook) { pGUI->AddToDrawingList(pCook->getItem()); pCook = pCook->getNext(); }
...
void Restaurant::AssignVIPOrders(int currentTime)
{
    ...
    while (!waitVIP.isEmpty())
    {
        ...
        if (assignedCook)
        {
            ...
            inService.InsertEnd(vipOrder); // logic correction
...
void Restaurant::preemptOrder(Cook* cook, Order* order, int currentTime)
{
    ...
    // Remove from inService list (as it was there)
    inService.DeleteNode(order);
...
void Restaurant::AssignNormalOrders(int currentTime)
{
    ...
    while (!waitNormal.isEmpty())
    {
        ...
        if (assignedCook)
        {
            ...
            inService.InsertEnd(normalOrder); // logic correction
...
void Restaurant::AssignVeganOrders(int currentTime)
{
    while (!waitVegan.isEmpty())
    {
        ...
        if (assignedCook)
        {
            ...
            inService.InsertEnd(veganOrder); // Logic correction
